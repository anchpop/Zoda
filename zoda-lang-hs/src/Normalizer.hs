...
do_fst :: JustifiedModule Plain ph i m n -> Semantic () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n
...
do_snd :: (Constraints m n) => JustifiedModule Plain ph i m n -> Semantic () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n
...
do_ap :: (Constraints m n) => JustifiedModule Plain ph i m n -> Semantic () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n
...
do_add :: JustifiedModule Plain ph i m n -> Semantic () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n
...
do_clos :: forall m ph i n. (Constraints m n)
...
mk_var :: Semantic t p ph i n -> Atom -> Semantic t p ph i n
mk_var tp atom = NeutralSem tp (VarSem atom)
 
 
-- |This converts the surface syntax into a semantic term with no beta redexes.
eval :: forall ph i m n. (Constraints m n) => JustifiedModule Plain ph i m n -> ExpressionX Plain () (Map.Key ph m) -> SemanticEnv () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n
eval modu = eval'
  where 
    eval' :: ExpressionX Plain () (Map.Key ph m) -> SemanticEnv () (Map.Key ph m) n -> Semantic () (Map.Key ph m) n
    ...
    eval' (ReferenceVariable _ m                                   _ _) env = getValue referent
...
--   - one for types. 
read_back_nf :: (Constraints m n) => JustifiedModule Plain ph i m n ->  Nf () (Map.Key ph m) n -> ExpressionX Plain () (Map.Key ph m)
...
read_back_tp :: forall m ph i n. (Constraints m n) => JustifiedModule Plain ph i m n -> Semantic () (Map.Key ph m) n -> ExpressionX Plain () (Map.Key ph m)
...
read_back_ne :: forall m ph i n.  (Constraints m n) => JustifiedModule Plain ph i m n -> Ne () (Map.Key ph m) n -> ExpressionX Plain () (Map.Key ph m)
...
make_initial_env :: forall ph i m n. (Constraints m n) => JustifiedModule Plain ph i m n -> [(Atom, ExpressionX Plain () (Map.Key ph m))] -> SemanticEnv () (Map.Key ph m) n
make_initial_env _    [] = []
...
{-
make_initial_env :: forall ph i m n. (Constraints m n) => JustifiedModule () () ph m () i n -> [(Atom, ExpressionX Plain () (Map.Key ph m))] -> SemanticEnv () (Map.Key ph m) n
make_initial_env _    [] = []
make_initial_env modu ((atom, t):env) = (atom, d):env'
  where
    env' :: SemanticEnv () (Map.Key ph m) n
...
normalize :: forall i m ph n. (Bindable i, Constraints m n) => 
  JustifiedModule Plain ph i m n -> SurfaceEnv i (Map.Key ph m) -> ExpressionX Plain i (Map.Key ph m) -> ExpressionX Plain () (Map.Key ph m) -> ExpressionX Plain () (Map.Key ph m)
normalize modu env term tp = read_back_nf modu' (Normal tp' term')
  where env'  :: SemanticEnv () (Map.Key ph m) n
        ...
        modu' = fmap plainToNormalizedPlain modu


normalizeType :: forall i m ph n. (Bindable i, Constraints m n) => 
  JustifiedModule Plain ph i m n -> SurfaceEnv i (Map.Key ph m) -> ExpressionX Plain i (Map.Key ph m) -> ExpressionX Plain () (Map.Key ph m)
