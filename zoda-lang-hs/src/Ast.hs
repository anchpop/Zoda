...
data Expression p = NumberLiteralExpression (NumberLiteral p) p | IdentifierExpression (LowercaseIdentifier p) p | FunctionLiteralExpression (FunctionLiteral p) p | FunctionApplicationExpression (Expression p) (Expression p) p deriving (Show, Read, Eq, Ord)
