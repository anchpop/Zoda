...
copyPropagated :: forall i p t o. (Show i, Show t, Show p, NominalShow i, NominalShow t, NominalShow p, Ord i, Bindable i, Bindable p, Bindable t) => [(i, DelcarationInfo t p (i, p) i i)] -> Module t p (i, p) i -> (forall ph. JustifiedModule t p ph i i i -> o) -> Either (ProductionError t p (i, p) i) o
copyPropagated prims (Module _ declarations _) f = Map.withMap dUMap (\m -> f <$> dJmapToJustifiedModule m)
  where
    ...
    dJmapToJustifiedModule :: (Map.Map ph i (DelcarationInfo t p (i, p) i i')) -> Either (ProductionError t p (i, p) i) (JustifiedModule t p ph i i i')
...
primatives :: [(Text, DelcarationInfo Untyped SourcePosition m i Text)]
primatives = [
